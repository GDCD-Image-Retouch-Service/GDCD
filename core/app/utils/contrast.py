...
def maxEntropyEnhance(I, isBad, a=-0.3293, b=1.1258):
    ...
    isBad = isBad * 1
    isBad = cv2.resize(isBad.astype('float32'), dsize=(50, 50), interpolation=cv2.INTER_CUBIC)
    isBad[isBad<0.5] = 0
    isBad[isBad>=0.5] = 1
    Y = Y[isBad==1]
...
def Ying_2017_CAIP(img, mu=0.5, a=-0.3293, b=1.1258):
    ...
    t_b = np.max(I, axis=2)
    t_our = cv2.resize(tsmooth(cv2.resize(t_b, dsize=(t_b.shape[1], t_b.shape[0]), interpolation=cv2.INTER_CUBIC), lamda, sigma), (t_b.shape[1], t_b.shape[0]), interpolation=cv2.INTER_AREA)
